...
| A-12 | R-09 | Both budgets use one concise layout without appended contract/handoff prose; context prioritizes warnings, labels truncation, and does not run a checker or write a record. Projections of an immutable snapshot perform no additional reads, and a new snapshot refreshes changed state. | Reminder performs work, hides critical state, mixes reads, or grows without bound; fix snapshot/renderer ownership. |
...
| Case | Requirements | Scenario and observable result | Failure detected; response |
|---|---|---|---|
| A-31 | R-18, R-10 | No selection returns project-only context; invalid selection preserves project guidance and the CLI error; missing project files leave task context usable and nonexistent orientation files are omitted. | Task failures erase orientation; fix independent loading. |
| A-32 | R-18, R-09 | Fresh reads reflect corrected guidance; fenced headings, duplicate sections, whole-section authoring placeholders, unreadable and escaping files are not treated as valid guidance; inequalities and standalone URI/email autolinks are retained. | Stale or ambiguous claims injected; fix explicit section reading. |
| A-33 | R-16, R-18 | Long paths and handoffs retain real guidance, warning, blocker and next-step content at 2000 characters; short reminders omit duplicate handoff prose; every warning is labeled, project diagnostics retain reasons without repeating the root, and JSON summaries remain bounded. | Sources consume useful content; fix allocation. |
...
| Case | Requirements | Scenario and observable result | Failure detected; response |
|---|---|---|---|
| A-37 | R-20, R-03, R-11 | A contract with a complete Question alignment section accepts a readable project-relative source and rejects duplicate/unknown/missing fields, near-miss headings, misplaced field labels, absolute/external/escaping paths, and missing/unreadable sources before begin. Legacy records with pre-v0.5 alignment-like sections remain readable and amendable with null alignment plus a warning. | Ambiguous lineage becomes accepted structure or new grammar invalidates retained history; separate strict acceptance from record compatibility without judging scientific truth. |
| A-38 | R-20, R-09, R-18 | Status and both reminder budgets read the governing question/alignment from the retained contract, keep realistic one/two-sentence declarations intact ahead of long mutable guidance, explicitly truncate extreme fields, bound JSON, and retain visible project guidance/warnings/blockers/next actions. A drifted working contract cannot replace the accepted question; source paths identify the retained agreement. | Mutable summaries, wrong source paths, or equal-share truncation displace the agreement; fix retained-source projection and staged allocation. |
