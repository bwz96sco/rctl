...
2. The release host evidence covers invocation-local Codex CLI 0.153.4 configuration, SessionStart startup and UserPromptSubmit. The new isolated project-file probe delivered no reminder; root cause remains undetermined. Official online retrieval succeeded during M3 after the historical preparation timeout. Real compact/native resume, subagent delivery, and ordinary persisted project installation are not established here.
