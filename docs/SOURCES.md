...
| S-08 | [retired research-project-setup snapshot](evidence/m5/retired-setup.md) and [migrated research control entry template](../templates/project/research/README.md) | Static scientific controls and task-local live state have distinct owners. | Preserve research knowledge ownership and local project paths. |
...
Local source paths identify the inspected originals outside this repository. They are provenance references, not navigable repository links; the decision-relevant observations above are self-contained. No private metrics, session transcripts, or global configuration have been copied into this project.
