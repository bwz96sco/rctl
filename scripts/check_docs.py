...
def main() -> None:
    ...
    assert len(requirements) == len(set(requirements)) == 20
    assert len(cases) == len({case[0] for case in cases}) == 40
    ...
    links = check_links()
    print(f"PASS preparation documents: {links} local links; {len(json_files)} JSON files; 4 schemas and example inputs; 20 requirements mapped to 40 acceptance cases.")
