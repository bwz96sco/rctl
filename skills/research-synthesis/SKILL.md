...
3. **Compare by issue.** Organize the reasoning by shared mechanism, assumption, setting, or result rather than paper-by-paper summary. Build comparison axes from the notes' field maps only when they improve the answer; a paper the axes cannot place shows that the map is incomplete.
4. **Trace the argument.** Every material synthesis claim names the contributing paper IDs and note anchors and preserves their evidence basis and caveats. Distinguish paper-reported findings from cross-paper analyst inferences; for an inference, state the comparison that supports it and the conditions under which it may fail.
